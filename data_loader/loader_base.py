...
class DataLoaderBase(object):
    ...
    def load_cf(self, filename):
        if  'reasoner' in filename:
            ...
            with open(filename, 'r',encoding='utf-8') as f:
                for line in f:
                    # Format: [user_id]\t[item list]
                    user_id, item_ids = line.strip().split("\t")
                    if len(item_ids) > 0:
                        for item_id in eval(item_ids):
                            user.append(int(user_id))
                            item.append(item_id)
                    user_dict[int(user_id)] = eval(item_ids)
                ...
                for line in f:
                    ...
                    if len(item_ids) > 0:
                        for item_id in eval(item_ids):
                            ...
                            item.append(item_id)
                    user_dict[int(user_id)] = eval(item_ids)
        elif 'luxury_beauty' in filename:
            user = []
            item = []
            user_dict = dict()
            with open(filename, 'r',encoding='utf-8') as f:
    ...
    def load_reasoner_kg(self, filename):
        kg = []
        with open(filename, 'r', encoding='utf-8') as f:
            for line in f:
                # Format: [head_entity_global_id]\t[relation_global_id]\t[tail_entity_global_id]
                cells = line.strip().split("\t")
                cells = [int(idd) for idd in cells]
                kg.append(cells)
        kg_data = pd.DataFrame(kg, columns=['h', 'r', 't'])
        return kg_data
    ...
    def load_pretrained_data(self):
        ...
        if  self.data_name not in ['reasoner','software','luxury_beauty']:
            pretrain_data = np.load(pretrain_path)
            self.user_pre_embed = pretrain_data['user_embed']
            self.item_pre_embed = pretrain_data['item_embed']
        elif self.data_name == 'reasoner':
            embed_file = "./datasets/reasoner/transe_embed.pkl"
            print('Load embedding:', embed_file)
            embed = pickle.load(open(embed_file, 'rb'))
            self.user_pre_embed = embed['user']
            self.item_pre_embed = embed['product']
        elif self.data_name == 'software':
            embed_file = "./datasets/software/transe_embed.pkl"
            print('Load embedding:', embed_file)
            embed = pickle.load(open(embed_file, 'rb'))
            self.user_pre_embed = embed['user']
            self.item_pre_embed = embed['product']
        elif self.data_name == 'luxury_beauty':
            embed_file = "./datasets/luxury_beauty/transe_embed.pkl"
            print('Load embedding:', embed_file)
            embed = pickle.load(open(embed_file, 'rb'))
            self.user_pre_embed = embed['user']
            self.item_pre_embed = embed['product']
